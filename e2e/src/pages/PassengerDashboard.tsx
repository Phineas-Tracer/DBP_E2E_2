  ...
  useEffect(() => {
    ...
    async function loadTrips() {
      try {
        ...
        if (active) setError('No pudimos cargar tus viajes. Intenta de nuevo.');
      } finally {
  ...
  return (
    <div className="dashboard passenger-dashboard">
      ...
      {loading ? (
        ...
        <EmptyState message="Aún no has pedido ningún viaje. Toca 'Pedir viaje' para empezar." />
      ) : (
...
/**
 * Interfaces asumidas (ajusta el import/nombre si difiere en tu proyecto):
 * - useAuth() -> { user: User | null, ... }
 * - tripService.getTrips(): Promise<Trip[]>
